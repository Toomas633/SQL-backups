...
def change_password():
    ...
    if bcrypt.check_password_hash(password_hash, current_password):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE users SET password = ? WHERE user = ?', (new_password_hash, username))
            conn.commit()
            conn.close()
...
def change_settings():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            'UPDATE settings SET value = ? WHERE setting = "dumps"', (dumps,))
        conn.commit()
        cursor.execute(
            'UPDATE settings SET value = ? WHERE setting = "cron"', (cron,))
        conn.commit()
        conn.close()
        return jsonify({"success": True})
    except sqlite3.Error as e:
...
def add_connection():
    ...
    if (data.get('port')):
        address = address + ':' + data.get('port')
    username = data.get('username')
    password = data.get('password')
    if (type == 'mysql'):
        status = functions.check_connection(name, type, address, username, password)
        if (status == True):
            return jsonify({"success": True})
        else:
